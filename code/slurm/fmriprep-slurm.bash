...
singularity run --cleanenv -B $SLURM_TMPDIR/fmriprep-lts:/WORK -B $HOME/.cache/templateflow:/templateflow -B /etc/pki:/etc/pki/ \
    $SLURM_TMPDIR/fmriprep-lts/envs/fmriprep-20.2.1lts.sif \
...
if [ $fmriprep_exitcode -eq 0 ] ; then
    cp -R $SLURM_TMPDIR/fmriprep-lts/inputs/openneuro/$DATASET/derivatives/fmriprep $SCRATCH/fmriprep_$DATASET_$SLURM_ARRAY_TASK_ID
    cp $SLURM_TMPDIR/fmriprep-lts/fmriprep_work/fmriprep_wf/resource_monitor.json $SCRATCH/fmriprep_$DATASET_$SLURM_ARRAY_TASK_ID/fmriprep_$DATASET-$PARTICIPANT_resource_monitor_$SLURM_ARRAY_TASK_ID.json 
fi 
